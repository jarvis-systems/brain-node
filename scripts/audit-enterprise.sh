...
log "${BOLD}[1/36] PHP syntax check${NC}"
...
log "${BOLD}[2/36] PHPUnit tests${NC}"
...
log "${BOLD}[3/36] Silent catch blocks${NC}"
...
log "${BOLD}[4/36] Debug artifacts${NC}"
...
log "${BOLD}[5/36] TODO/FIXME markers${NC}"
...
log "${BOLD}[6/36] Unsafe patterns (eval/shell_exec/die/exit)${NC}"
...
log "${BOLD}[7/36] Shell script safety headers${NC}"
...
log "${BOLD}[8/36] No-op escape method detection${NC}"
...
log "${BOLD}[9/36] Late static binding in traits${NC}"
...
log "${BOLD}[10/36] Known typos in codebase${NC}"
...
log "${BOLD}[11/36] Dev dependencies in production require${NC}"
...
log "${BOLD}[12/36] PHPStan static analysis${NC}"
...
log "${BOLD}[13/36] Missing declare(strict_types=1)${NC}"
...
log "${BOLD}[14/36] Secret patterns in tracked files${NC}"
...
log "${BOLD}[15/36] Hardcoded user paths in tracked source files${NC}"
...
log "${BOLD}[16/36] Degradation observability in catch blocks${NC}"
...
log "${BOLD}[17/36] Version consistency${NC}"
...
log "${BOLD}[18/36] MCP schema bypass enforcement${NC}"
...
log "${BOLD}[19/36] Compile clean-worktree gate${NC}"
...
log "${BOLD}[20/36] Agent schema enabled set${NC}"
...
log "${BOLD}[21/36] MCP tool policy contract${NC}"
...
log "${BOLD}[22/36] Self-hosting workspace hygiene${NC}"
...
log "${BOLD}[23/36] Test Mode Contract enforcement${NC}"
...
log "${BOLD}[24/36] MCP policy inspector output contract${NC}"
...
log "${BOLD}[25/36] MCP allowlist contract${NC}"
...
log "${BOLD}[26/36] mcp:docs-search contract${NC}"
...
log "${BOLD}[27/36] mcp:diagnose contract${NC}"
...
log "${BOLD}[28/36] Secret reference hygiene${NC}"
...
log "${BOLD}[29/36] MCP registry contract${NC}"
...
log "${BOLD}[30/36] MCP compile consistency${NC}"
...
log "${BOLD}[31/36] MCP registry validation${NC}"
...
log "${BOLD}[32/36] MCP call bridge v1${NC}"
...
log "${BOLD}[33/36] MCP external tools policy${NC}"
...
log "${BOLD}[34/36] MCP external tools call gating${NC}"
...
log "${BOLD}[35/36] MCP discovery list${NC}"
...
log "${BOLD}[36/36] MCP discovery describe${NC}"
