...
OUTPUT=$(BRAIN_TEST_MODE=1 php cli/bin/brain mcp:call --server=sequential-thinking --tool=think --input='{"thought":"hello"}' --trace)
if ! echo "$OUTPUT" | jq -e '.request_id' >/dev/null; then
    echo "FAIL: mcp:call --trace missing request_id"
    exit 1
fi
if ! echo "$OUTPUT" | jq 'has("redactions_applied")' | grep -q "true"; then
...
OUT1=$(BRAIN_TEST_MODE=1 php cli/bin/brain mcp:call --server=sequential-thinking --tool=think --input='{"thought":"hello"}' --trace)
OUT2=$(BRAIN_TEST_MODE=1 php cli/bin/brain mcp:call --server=sequential-thinking --tool=think --input='{"thought":"hello"}' --trace)
