...
class TasksCommand extends CommandArchetype
{
    protected function handle(): void
    {
        // Role
        $this->guideline('role')
            ->text('Dynamic agent discovery via Bash(brain master:list). Read parent Phase Issue(s) via @agent-pm-master. Iterative loop (ONE task at a time). Sequential multi-agent research chain per task. Brain synthesis with rich context (Memory IDs, .docs/ refs). Delegate execution to @agent-pm-master.');
