...
## Examples
 ...
 - **extract**: Extract: query (required), category?, limit?, offset?, tags?
 - **defaults**: Defaults: limit=10, offset=0
 - **output**: STORE-AS(var PARAMS = '{query, category?, limit, offset?, tags?}')
</guideline>
<guideline id="workflow-step2">
STEP 2 - Execute Semantic Search
## Examples
 - **search**: mcp__vector-memory__search_memories('STORE-GET(var PARAMS)')
 - **store**: STORE-AS(var RESULTS = 'search results array')
</guideline>
<guideline id="workflow-step3">
STEP 3 - Handle Empty Results
## Examples
 - **check**: IF(STORE-GET(var RESULTS) is empty) → THEN → [Display: "No memories found for: {query}" → Suggest: "Try broader search terms" → Suggest: "Remove category/tag filters" → Suggest: "Use /mem:list to see recent memories"] → END-IF
</guideline>
...
## Examples
 ...
 - **list**: FOREACH(memory in STORE-GET(var RESULTS)) → [Display: "#{id} [{category}] (similarity: {score})" → Display: "  {content_preview} (first 100 chars)" → Display: "  Tags: {tags} | Accessed: {access_count}x"] → END-FOREACH
