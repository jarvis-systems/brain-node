...
## Examples
 ...
 - STORE-AS(var PRIOR_SOLUTIONS = 'Relevant past approaches')
...
## Examples
 - FOREACH(scan_target in $REQUIREMENTS_PLAN.scan_targets) → [Context extraction from {scan_target} → STORE-AS(var GATHERED_MATERIALS[{target}] = 'Extracted context')] → END-FOREACH
 - IF($DOCS_SCAN_NEEDED === true) → THEN → [Bash(brain docs {keywords}) → [Find documentation index (returns: Path, Name, Description)] → END-Bash → STORE-AS(var DOCS_INDEX = 'Documentation file index') → FOREACH(doc in $DOCS_INDEX) → [Read('{doc.path}')] → END-FOREACH → STORE-AS(var DOCS_SCAN_FINDINGS = 'Documentation content')] → END-IF
 - IF($WEB_RESEARCH_NEEDED === true) → THEN → [WebSearch(Research best practices for {$TASK}) → STORE-AS(var WEB_RESEARCH_FINDINGS = 'External knowledge')] → END-IF
...
## Examples
 ...
 - STORE-AS(var CONTEXT = '{files_found, code_patterns, current_state}')
 - Create atomic execution plan: specific edits with exact changes
 - STORE-AS(var PLAN = '[{step_N, file, action: read|edit|write, description, exact_changes}, ...]')
 ...
 - IF($IS_VECTOR_TASK === true) → THEN → [mcp__vector-task__task_update('{task_id: $VECTOR_TASK_ID, status: "in_progress", comment: "Execution started after plan approval", append_comment: true}') → OUTPUT(📋 Vector task #{$VECTOR_TASK_ID} started)] → END-IF
</guideline>
...
## Examples
 ...
 - FOREACH(step in $PLAN) → [OUTPUT(▶️ Step {N}: {step.description}) → IF(step.action === "read") → THEN → [Read('{step.file}') → STORE-AS(var FILE_CONTENT[{N}] = 'File content')] → END-IF → IF(step.action === "edit") → THEN → [Read('{step.file}') → Edit('{step.file}', '{old_string}', '{new_string}')] → END-IF → IF(step.action === "write") → THEN → [Write('{step.file}', '{content}')] → END-IF → STORE-AS(var STEP_RESULTS[{N}] = 'Result') → OUTPUT(✅ Step {N} complete)] → END-FOREACH
...
## Examples
 - STORE-AS(var SUMMARY = '{completed_steps, files_modified, outcome}')
...
## Examples
 ...
 - **detection**: Pattern matched → task_get(5) → Load task + parent
 - **context**: Task: "Fix null check in helper" | Parent: "Bug fixes sprint"
 - **flow**: Task Detection → Context → Plan ✓ → task_start → Execute → task_finish → Complete
 - **result**: Vector task #5 completed ✓
</guideline>
